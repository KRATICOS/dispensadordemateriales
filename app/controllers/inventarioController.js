...
exports.actualizarEquipoConImagenes = async (req, res) => {
    try {
        ...
        if (nseries && nseries !== equipo.nseries) {
            ...
            if (existeSerie) return res.status(400).json({ message: 'Número de serie ya registrado en otro equipo' });
        }
...
exports.obtenerPorCategoria = async (req, res) => {
    try {
        ...
        if (!equipos || equipos.length === 0) return res.status(404).json({ message: 'Categoría no encontrada' });
        res.status(200).json(equipos);
    } catch (error) {
